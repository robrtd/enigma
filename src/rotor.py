...
ALPHA_LABELS = string.ascii_uppercase[0:ALPHABET_LENGTH]
...
class Rotor:
    ...
    def __init__(self, model_name, wiring, ring_setting=0, stepping=None):
        ...
        # check wiring length
        if len(self.wiring_str) != ALPHABET_LENGTH:
        ...
        if not isinstance(ring_setting, int) or not (0 <= ring_setting < ALPHABET_LENGTH):
        ...
        self.exit_map = [0] * ALPHABET_LENGTH
        ...
        for n in range(ALPHABET_LENGTH):
            self.display_map[chr(ord('A') + n)] = (n - self.ring_setting) % ALPHABET_LENGTH
    ...
    def signal_out(self, n):
        ...
        contact = (n + self.pos) % ALPHABET_LENGTH

        # run it through the internal wiring
        pin = self.exit_map[contact]

        # turn back into a position due to rotation
        return (pin - self.pos) % ALPHABET_LENGTH
    ...
    def rotate(self):
        ...
        self.pos = (self.pos + 1) % ALPHABET_LENGTH
